          ...
          <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <Transition enter-active-class="transition-all duration-150" enter-from-class="opacity-0 -translate-y-1"
              ...
              <div v-if="selectedParts.length > 0"
                class="px-4 py-2 bg-blue-50 border-b border-blue-100 flex items-center justify-between gap-3">
                <span class="text-sm font-medium text-blue-900">
                  {{ selectedParts.length }} part{{ selectedParts.length > 1 ? 's' : '' }} selected
                </span>
                <div class="flex items-center gap-2">
                  <button type="button" @click="clearSelection"
                    class="text-xs text-blue-600 hover:text-blue-800 px-2 py-1 transition-colors">
                    Clear
                  </button>
                  <button type="button" @click="addSelectedToCart" :disabled="bulkAdding"
                    class="inline-flex items-center gap-1.5 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white text-sm px-3 py-1.5 rounded-lg transition-colors">
                    <span v-if="bulkAdding" class="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin"></span>
                ...
                <tbody>
                  <template v-for="part in parts" :key="part.part_id">
                    ...
                    <tr
                      ...
                      :class="[
                        isSelected(part) ? 'bg-blue-50 border-blue-200' : 'hover:bg-gray-50',
...
const selectedPartIds = ref(new Set())
...
const selectedParts = computed(() =>
  parts.value.filter(p => selectedPartIds.value.has(p.part_id) && p.rrp != null && p.number)
)
...
function isSelected(part) {
  return selectedPartIds.value.has(part.part_id)
}

function selectPart(part) {
  if (selectedPartIds.value.has(part.part_id)) {
    selectedPartIds.value.delete(part.part_id)
  } else {
    selectedPartIds.value.add(part.part_id)
  }
...
function clearSelection() {
  selectedPartIds.value.clear()
}

async function addSelectedToCart() {
  if (bulkAdding.value || selectedParts.value.length === 0) return
